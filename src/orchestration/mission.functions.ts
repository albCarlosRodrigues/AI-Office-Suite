...
async function resetDurableMissionForRetry(missionId: string) {
  const store = runtimeDurableStore();

  await store.transaction((state) => {
    const oldRequests = state.toolRequests.filter((request) => request.missionId === missionId);

    const toolCallIds = new Set(oldRequests.map((request) => request.toolCallId));

    const idempotencyKeys = new Set(oldRequests.map((request) => request.idempotencyKey));

    state.missions = state.missions.filter((mission) => mission.id !== missionId);

    state.tasks = state.tasks.filter((task) => task.missionId !== missionId);

    state.toolRequests = state.toolRequests.filter((request) => request.missionId !== missionId);

    state.toolResults = state.toolResults.filter((result) => !toolCallIds.has(result.toolCallId));

    state.approvals = state.approvals.filter((approval) => approval.missionId !== missionId);
...
export const retryMission = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await resetDurableMissionForRetry(data.missionId);
