...
export function parsePrxReplyCandidate(
  ...
  const normalize = (reply: CandidateReply) => {
    if (
    ...
    } else if (reply.text !== null && reply.text !== undefined) {
      try {
    ...
    if (!replyText || replyText === "YOUR RESPONSE HERE") {
      return null;
    }
  ...
  const recoverMalformedStringEnvelope = (candidate: string) => {
    /*
    ...
    const tail = candidate.slice(markerIndex + marker.length).trim();

    if (!tail.endsWith('"}')) {
      return null;
    }

    const rawText = tail.slice(0, -2).trim();
    ...
    try {
      const metadata = JSON.parse(`${prefix}"text":""}`) as CandidateReply;
  ...
  /*
   * Recovery for structured PRX replies where
   * text is an object/array but the model emitted
   * an invalid JSON escape inside that payload,
   * typically a Windows path such as:
   *
   * A:\Ambiente\Projeto
   *
   * Metadata is parsed independently so transport
   * correlation is never bypassed.
   */
  const recoverMalformedObjectEnvelope = (candidate: string) => {
    const marker = '"text":';
    ...
    /*
     * Last } belongs to the outer PRX envelope.
     * Everything before it is the raw text payload.
     */
    if (!tail.endsWith("}")) {
      return null;
    }

    const rawPayload = tail.slice(0, -1).trim();
    ...
    try {
      const metadata = JSON.parse(prefix + '"text":null}') as CandidateReply;
      ...
      try {
        ...
        /*
         * Repair only INVALID JSON escapes.
         *
         * Example:
         *   A:\Ambiente
         *
         ...
         *   A:\\Ambiente
         *
         * Existing valid JSON escapes are preserved.
         */
        const repaired = rawPayload.replace(/\\(?!["\\/bfnrtu])/g, "\\\\");
  ...
  for (const candidate of attempts) {
    try {
      const reply = JSON.parse(candidate) as CandidateReply;
      ...
      const recovered =
        recoverMalformedStringEnvelope(candidate) ?? recoverMalformedObjectEnvelope(candidate);
...
export class CdpPrxSessionClient implements PrxSessionClient {
  ...
  private async connect(signal: AbortSignal) {
    ...
    if (!target) {
      throw new Error("PRX_SESSION_DISCONNECTED");
    }
  ...
  private composerSource() {
    const preferred = this.composerLabel.trim();
  ...
  private async getUiState(signal: AbortSignal): Promise<DesktopUiState> {
    const source = this.composerSource();

    const state = await this.evaluate<DesktopUiState>(
      `(() => {
          ${source}
    ...
    if (!state) {
      throw new Error("PRX_UI_STATE_UNAVAILABLE");
    }
   ...
   */
  private async ensureNormalChat(signal: AbortSignal) {
    let state = await this.getUiState(signal);

    if (state.workActive) {
      throw new Error("PRX_WORK_MODE_BLOCKED");
    }
    ...
    if (!clicked) {
      throw new Error("PRX_CHAT_MODE_NOT_FOUND");
    }

    await delay(300, undefined, { signal });

    state = await this.getUiState(signal);

    if (state.workActive || (state.modeToggleExists && !state.chatActive)) {
      throw new Error("PRX_CHAT_MODE_NOT_ACTIVE");
    }
  }
  ...
  private async ensureConversation(signal: AbortSignal) {
    ...
    if (state.title === conversationTitle || state.selectedConversation === conversationTitle) {
      return;
    }

    const project = (await this.evaluate<{
      found: boolean;
      expanded: boolean;
    }>(
      `(() => {
    ...
    if (!opened?.ok) {
      throw new Error("PRX_CONVERSATION_CLICK_FAILED");
    }
    ...
    for (let attempt = 0; attempt < 50; attempt++) {
      ...
      if (
        (state.title === conversationTitle || state.selectedConversation === conversationTitle) &&
  ...
  private async prepareComposer(signal: AbortSignal) {
    const source = this.composerSource();
  ...
  async send(request: PrxRequest, signal: AbortSignal) {
    ...
    await this.ensureNormalChat(signal);
    ...
    if (state.generating) {
      throw new Error("PRX_ALREADY_GENERATING");
    }

    if (!state.composerExists) {
      throw new Error("PRX_COMPOSER_NOT_FOUND");
    }
    ...
    await this.prepareComposer(signal);
    ...
    for (let attempt = 0; attempt < 30; attempt++) {
      ...
      if (state.composerText.includes(request.requestId) && state.sendReady) {
        readyToSend = true;
        break;
      }
    }
    ...
    if (!readyToSend) {
      throw new Error("PRX_SEND_NOT_READY");
    }
    ...
    if (!sent?.ok) {
      throw new Error(
        sent?.reason === "send_disabled" ? "PRX_SEND_DISABLED" : "PRX_SEND_BUTTON_NOT_FOUND",
      );
    }
    ...
    for (let attempt = 0; attempt < 40; attempt++) {
      ...
      if (state.generating || !state.composerText.includes(request.requestId)) {
        committed = true;
        break;
      }
    }
    ...
    if (!committed) {
      throw new Error("PRX_SEND_NOT_COMMITTED");
    }
  }
  ...
  async receive(request: PrxRequest, signal: AbortSignal) {
    ...
    const state = await this.getUiState(signal);

    if (
      state.title !== this.conversationTitle.trim() &&
      state.selectedConversation !== this.conversationTitle.trim()
    ) {
      throw new Error("PRX_CONVERSATION_CHANGED");
    }
